...
exports.getMe = async (req, res) => {
    try {
        const info = parseJwt(req.cookies.token)
        const user = await User.findOne(users, { where: info.id });
...
exports.register = async (req, res) => {
    ...
    try {
        ...
        console.error(err);
        return res.status(400).json({ error: 'Some error have occured' });
    }
...
exports.confirmEmail = async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            return res.send('Email has been successfully confirmed');
        }
        catch(err) {
            console.error(err);
        ...
        return res.status(400).json({ error: 'This verification link is no longer reachable' });
    }
}
...
exports.login = async (req, res) => {
    try {
        ...
        if(!passCheck) {
            return res.status(400).json({ error: 'Login or password is incorrect' });
        }
